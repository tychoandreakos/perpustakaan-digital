            ...
            <div class="card shadow">
                <div class="card-header border-0">
                    <div class="row align-items-center">
                        <div class="col">
                            <h3 class="mb-0">Daftar Lokasi Rak Buku</h3>
                        </div>
                        <div class="col text-right">
                            <a :href="this.route" class="btn btn-sm btn-primary"><i
                                    class="ni ni-fat-add text-white"></i> Tambah Lokasi Rak Buku</a>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table align-items-center table-flush text-center">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th scope="col">Kode Lokasi Rak Buku</th>
                                    <th scope="col">Nama Lokasi Rak Buku</th>
                            ...
                            <tbody>
                                <tr v-for="item in datas.data" :key="item.id">
                                    ...
                                    <td>
                                        {{ item.kode_lokasi | capitalize}}
                                    </td>
                                    <td>
                                        {{ item.nama_lokasi | capitalize }}
                                    </td>
                                    <td>
                                        {{ item.updated_at }}
                                    </td>
    ...
    export default {
        ...
        methods: {
            ...
            deleted(val) {
                this.$swal({
                    ...
                    if (result.value) {
                        axios.post('/pustakawan/lokasi/' + val, {
                                _method: 'DELETE'
                            })
                            .then(res => {
                                this.$swal({
        ...
        created() {
            ...
            Fire.$on('searching', () => {
                ...
                axios.get('/pustakawan/lokasi-search?q=' + query)
                    .then(res => {
                        // console.log(res)
                        this.datas = res.data
                    })
